makeProfiles=function(flist,genChar){
	...
	for(f in names(flist)){
		con=file(flist[[f]],open="r")
		lskip=1
		while(!grepl("##########",readLines(con,n=1,warn=FALSE))) lskip=lskip+1
		close(con)
		dfs[[f]]=read.delim(flist[[f]],skip=lskip,stringsAsFactors=FALSE,header=TRUE,sep="\t")
	}
...
plotSimilarFit=function(targ,prof,nearest=c(),farthest=c(),ylim=c()){
	...
	nearprof=prof[rownames(prof)%in%names(nearest),1:nExp]
	farprof=prof[rownames(prof)%in%names(farthest),1:nExp]
	...
	# Get order of boxes sorted out
	cat(colnames(prof[,1:nExp]))
